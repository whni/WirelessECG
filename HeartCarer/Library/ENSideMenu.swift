...
public class ENSideMenu : NSObject {
    ...
    public init(sourceView: UIView, menuPosition: ENSideMenuPosition) {
        ...
        // Add left edge swipe gesture recognizer
        let rightEdgeSwipeGestureRecognizer = UIScreenEdgePanGestureRecognizer()
        rightEdgeSwipeGestureRecognizer.addTarget(self, action: "handleEdgeGesture:")
        rightEdgeSwipeGestureRecognizer.edges = UIRectEdge.Left
        

        if (menuPosition == .Left) {
            // sourceView.addGestureRecognizer(rightSwipeGestureRecognizer)
            sourceView.addGestureRecognizer(rightEdgeSwipeGestureRecognizer)
    ...
    private func toggleMenu (shouldOpen: Bool) {
        ...
        if (bouncingEnabled) {
            ...
            if (menuPosition == .Left) {
                ...
                pushMagnitude = (shouldOpen) ? 120 : -120
        ...
        if (shouldOpen) {
            // reload menu table when showing up
            menuTableViewController.tableView.reloadData()
            let navigationController: UINavigationController = UIApplication.sharedApplication().keyWindow?.rootViewController as UINavigationController
            menuTableViewController.tableView.selectRowAtIndexPath(NSIndexPath(forRow: find(viewControllerSet, navigationController.viewControllers[0] as UIViewController)!, inSection: 0), animated: false, scrollPosition: UITableViewScrollPosition.None)
    ...
    // Swipe recognizer for the side menu
    ...
    // Swipe recognizer for the source view
    internal func handleEdgeGesture(gesture: UIScreenEdgePanGestureRecognizer) {
        // Not necessarily check the swipe edge
        if (gesture.state == UIGestureRecognizerState.Began) {
            // left edge response
            let navigationCtrl = UIApplication.sharedApplication().keyWindow?.rootViewController as UINavigationController
            if navigationCtrl.viewControllers.count == 1 {
                // show menu
                toggleMenu(self.menuPosition == .Left)
            } else {
    ...
    func updateMenuTableInset() {
        menuTableViewController.tableView.contentInset = UIEdgeInsetsMake((UIApplication.sharedApplication().keyWindow?.rootViewController as UINavigationController).navigationBar.frame.height + UIApplication.sharedApplication().statusBarFrame.height, 0, 0, 0)
    }
